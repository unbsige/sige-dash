...
def load_daily_data_with_cache(plant_key, device_id, selected_date):
...
def calculate_daily_metrics(df_power, df_energy, plant_config):
    if df_power.empty:
        return {
            "daily_energy": 0,
            "max_power": 0,
    ...
    if not df_energy.empty and "full_power_hours" in df_energy.columns:
        ...
        full_power_hours = (
            daily_energy / plant_config.get("capacity_kwp", 1) if plant_config.get("capacity_kwp", 0) > 0 else 0
        )

    daily_yield = daily_energy / plant_config["capacity_kwp"] if plant_config.get("capacity_kwp", 0) > 0 else 0

    peak_time = "N/A" if df_power.empty else df_power.idxmax()["pv_power"].strftime("%H:%M")

    capacity_factor = (
        (max_power / plant_config["capacity_kwp"]) * 100 if plant_config.get("capacity_kwp", 0) > 0 else 0
    )
...
def calculate_previous_day_metrics(plant_key, device_id, selected_date, plant_config):
    prev_date = selected_date - timedelta(days=1)
    prev_data = load_daily_data_with_cache(plant_key, device_id, prev_date)

    if not prev_data or prev_data["power"].empty:
        return {"daily_energy": 0, "max_power": 0, "full_power_hours": 0, "daily_yield": 0}

    return calculate_daily_metrics(prev_data["power"], prev_data["energy"], plant_config)
...
def create_daily_power_chart(df_power, selected_date, plant_config):
    if df_power.empty:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            hovertemplate="<b>%{x|%H:%M}</b><br><b>%{y:.1f} kW</b><extra></extra>",
            showlegend=False,
        )
    )
    ...
    if plant_config.get("capacity_kwp", 0) > 0:
        fig.add_hline(
            y=plant_config["capacity_kwp"],
            line_dash="dash",
            line_color="rgba(231, 76, 60, 0.7)",
            annotation_text=f"Capacidade: {plant_config['capacity_kwp']} kW",
    ...
    max_power = df_power["pv_power"].max()
    y_max = max(max_power * 1.15, plant_config.get("capacity_kwp", 0) * 1.1) if max_power > 0 else 10
...
def render_daily_detailed_statistics(df_power, daily_metrics, plant_config):
...
def show_daily_production(plant_key, device_id, plant_config):
    ...
    daily_data = load_daily_data_with_cache(plant_key, device_id, selected_date)

    if daily_data and not daily_data["power"].empty:
        df_power = daily_data["power"]
        df_energy = daily_data["energy"]

        daily_metrics = calculate_daily_metrics(df_power, df_energy, plant_config)
        prev_metrics = calculate_previous_day_metrics(plant_key, device_id, selected_date, plant_config)

        render_daily_metrics_cards(daily_metrics, prev_metrics, cols_cards)
        create_daily_power_chart(df_power, selected_date, plant_config)
        render_daily_detailed_statistics(df_power, daily_metrics, plant_config)
...
def clear_daily_cache():
    if "daily_cache" in st.session_state:
        ...
        st.success("Cache diário limpo com sucesso!")


def show_cache_info():
    cache = st.session_state.get("daily_cache", {})
    size = len(cache)
    st.info(f"📦 Cache diário: {size} dia(s)" if size else "📦 Cache diário vazio")
    if size and st.button("🗑️ Limpar Cache"):
        clear_daily_cache()
